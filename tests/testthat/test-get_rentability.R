...
test_that("get_rentability() works", {
  ...
  horizon <-opts_fast$parameters$general$horizon
  x <- list()
  x$invested_capacities <- antaresXpansion:::initiate_candidate_capacities(candidates, horizon)


  
  # launch function
  rentability <- antaresXpansion:::get_expected_rentability(opts_fast, current_it, candidates, n_w , x)
