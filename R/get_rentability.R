  ...
  else
  {
    ...
    get_aggr_rentability <- function(c)
    {
      ...
      {        
        ...
        {
          # if link_profile is constant during the 8760 hours, we prefer extract annual values from Antares (better rounding)          
          if(length(unique(c$link_profile))==1 && sum(subset(output_link_h_s_direct, mcYear == y)$sens) == 8736)
          {
          ...
          else
          {
            for(w in current_it$weeks)
            {
              ...
              last_h <- 7*24*w
              # if link_profile is constant during a week, we prefer extract weekly values from Antares (better rounding)
              if (length(unique(c$link_profile[first_h:last_h]))==1 && sum(subset(output_link_h_s_direct,mcYear == y )$sens[first_h:last_h]) == 168)
              {
              ...
              {
                tmp_rentability_direct <- tmp_rentability_direct + sum(as.numeric(subset(output_link_h_s_direct, link == c$link & timeId >= first_h & timeId <= last_h & mcYear == y)$"MARG. COST")*c$link_profile[first_h:last_h])
              }
            }
          }  
          
          ###### # Indirect :          
          if(length(unique(c$link_profile_indirect))==1 &&  sum(subset(output_link_h_s_indirect, mcYear == y)$sens) == 8736)
          {
          ...
          else
          {
            for(w in current_it$weeks)
            {
              ...
              last_h <- 7*24*w
              # if link_profile is constant during a week, we prefer extract weekly values from Antares (better rounding)
              if (length(unique(c$link_profile_indirect[first_h:last_h]))==1 && sum(subset(output_link_h_s_indirect,mcYear == y )$sens[first_h:last_h]) == 168)
              {
              ...
              {
                tmp_rentability_indirect <- tmp_rentability_indirect + sum(as.numeric(subset(output_link_h_s_indirect, link == c$link & timeId >= first_h & timeId <= last_h  & mcYear == y)$"MARG. COST")*c$link_profile_indirect[first_h:last_h])
              }
            }
          }
        }
