...
multi_year_investment <- function(path_solver, directory_path = getwd(), display = TRUE, report = TRUE, clean = TRUE, parallel = TRUE)
{
  ...
  for(stu in studies$opts)
  {
    ...
    candidates <- read_candidates(file = paste0(directory_path,"/candidates.ini"), stu)
  }
  ...
  for(s in 1:studies$n_simulated_years)
  {
    ...
    n_w[[s]] <- floor((studies$opts[[s]]$parameters$general$simulation.end - studies$opts[[s]]$parameters$general$simulation.start + 1)/7) # number of weeks 
    weeks[[s]] <- first_sim_week:(first_sim_week + n_w[[s]] - 1) # identifier of weeks to simulate for all expansion planning optimisation
    mc_years[[s]] <- getPlaylist(studies$opts[[s]]) # identifier of mc years to simulate for all expansion planning optimisation
    n_mc[[s]] <- length(mc_years[[s]]) # number of mc_years
    all_areas[[s]] <- antaresRead::getAreas(opts = studies$opts[[s]]) # all area of the first
  }
